        ...
        try:
            ...
            if not username:
                ...
                if len(req_parts) == 3:
                    ...
                    conn.send(
                        "Bad request: Format 'login <username> <password>' or 'register <username> <password>'".encode())
            # Phase 2: Handle authenticated commands
                ...
                if not os.path.exists(user_docs_dir):
                    conn.send(
                        f"Error: Your user data directory '{user_docs_dir}' does not exist. Please contact support.".encode())
                ...
                if command == 'upload':
                    ...
                    if safe_filepath is None:
                        conn.send(
                            f"Access denied: Cannot upload to '{filename_client}' outside your designated area.".encode())
                        continue
                    ...
                    if file_size > user_quota:
                        ...
                        file_logger.warning(
                            f"User {username} tried to upload {file_size} bytes, but only has {user_quota} bytes quota.")
                    else:
                        ...
                        with open(safe_filepath, 'wb') as f:
                            ...
                            while received_bytes < file_size:
                                ...
                                if not data:  # Client disconnected during upload
                                    file_logger.error(
                                        f"User {username} disconnected during upload of {filename_client}. Incomplete file.")
                            ...
                            if received_bytes == file_size:
                                file_logger.info(
                                    f"User {username} uploaded file: {safe_filepath} ({received_bytes} bytes)")
                                conn.send(f"File '{filename_client}' uploaded successfully.".encode())
                            else:
                                file_logger.error(
                                    f"User {username} upload of {filename_client} failed. Expected {file_size}, received {received_bytes}. Reverting quota.")
                                # Attempt to revert quota if upload was incomplete
                                users = load_users()
                                users[username]['quota'] += (file_size - received_bytes)  # Revert only the difference
                                save_users(users)
                                conn.send(
                                    f"Error: Incomplete upload for '{filename_client}'. Please try again.".encode())
                    ...
                    if safe_filepath is None:
                        conn.send(
                            f"Access denied: Cannot download '{filename_client}' from outside your designated area.".encode())
                        continue
            ...
            conn_logger.error(
                f"Unhandled error in handle_client for {username if username else addr} with request '{request if 'request' in locals() else 'N/A'}': {e}",
                exc_info=True)
            try:
