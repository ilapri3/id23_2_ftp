...
def receive_file(sock, filepath, file_size):
    ...
    try:
        ...
        with open(filepath, 'wb') as f:
            while received_bytes < file_size:
                ...
                if not data:  # Server disconnected or error during transfer
                    print(
                        f"Error: Server disconnected during download of '{os.path.basename(filepath)}'. Incomplete file.")
        ...
        if received_bytes == file_size:
            ...
            print(
                f"Error: Incomplete download for '{os.path.basename(filepath)}'. Expected {file_size}, received {received_bytes}.")
            # Optionally clean up incomplete file: os.remove(filepath)
            return False
    except socket.error as e:
